...
def run_upgrader(creep):
    target_refill_by_range = creep.pos.findClosestByPath(FIND_DROPPED_RESOURCES)
    ...
    if creep.store.getUsedCapacity() == 0:
        console.log(target_refill_by_range)
        console.log('upgrader log')
        creep.moveTo(target_refill_by_range)
        creep.pickup(target_refill_by_range)
